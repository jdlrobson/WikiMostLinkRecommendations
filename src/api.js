...
Let the search engine decide on the best profile to use.
  ...
  let profile = 'classic_noboostlinks';
  let searchQuery = `morelike%3A${mLlQuery}`;

  if ( moreLikeCache[mLlQuery] ) {
    return Promise.resolve( moreLikeCache[mLlQuery] );
  }
  return fetch(`https://en.m.wikipedia.org/w/api.php?action=query&formatversion=2&origin=*&format=json&smaxage=86400&maxage=86400&uselang=content&list=search&&srsearch=${searchQuery}&srnamespace=0&srlimit=6&srqiprofile=${profile}`)
    .then((r) => r.json())
    .then((j) => {
...
export default { mostLinks, moreLike };
